...
interface EditarMultaFormProps {
  multa: Multa;
  onClose: () => void;
...
export const EditarMultaForm: React.FC<EditarMultaFormProps> = ({ multa, onClose, onGuardar }) => {
  const [comuneroId, setComuneroId] = useState(multa.comuneroId);
  const [busqueda, setBusqueda] = useState(multa.comuneroNombre);
  ...
  const [asambleaFecha, setAsambleaFecha] = useState(
    multa.asamblea?.fecha ? multa.asamblea.fecha.slice(0, 10) : new Date().toISOString().slice(0, 10)
  );
      ...
      >
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 bg-slate-50/50 shrink-0">
          <div>
            ...
            <p className="text-sm text-gray-400 font-medium mt-0.5">Folio {multa.folio} · actualiza los datos necesarios.</p>
          </div>
        ...
        <div className="p-6 overflow-y-auto space-y-5">
            ...
            <div className="relative">
              ...
              <input
                ...
                onChange={(e) => {
                  ...
                  setMenuAbierto(true);
                  if (!e.target.value) { setComuneroId(''); }
                }}
          ...
          <div className="flex items-start gap-2.5 bg-[#F5F3F3] border border-[#E6E8EB] p-3.5 rounded-xl">
            ...
            <p className="text-sm text-[#5F5E5E] leading-relaxed font-medium">
              El estado y el historial de pago de la multa no se modifican al editar estos datos.
            </p>
          </div>
        </div>
